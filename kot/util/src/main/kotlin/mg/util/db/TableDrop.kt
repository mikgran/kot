...
class TableDrop {
    ...
    fun registerRelational(any: Any) {
        ...
        val uniqueKeys = uniquesByParent.keys.distinct()
        val uniqueValues = uniquesByParent.values.flatten().distinct()
        uniques.addAll(uniqueKeys)
        uniques.addAll(uniqueValues)
